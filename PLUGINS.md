...
confirmation, runs the optional `on_install` hook, and registers the plugin in
the main menu. On the next launch of `macrift`, the plugin's menu entry appears
under its declared section.
...
```json
{
  ...
  "lifecycle": {
    "on_install": "handlers/on_install.sh",
    "on_remove":  "handlers/on_remove.sh"
  }
}
```
...
4. **No mutating macrift's own state** — don't touch `$MACRIFT_*` variables,
   `~/.macrift/journal*` files, or the registry. Plugins are guests.
...
macrift plugin add github.com/x/y[@<ref>]
  ...
  → run lifecycle.on_install (if defined)         ─┐
                                                   │  output journaled
...
1. Runs `lifecycle.on_remove` if defined.
2. Replays the plugin's journal entries in reverse — undoes each
   `audit_default`, restores each `copy_config` backup, unloads each launchd
   label, strips each marker block from the user's rc files.
3. Removes `~/.macrift/plugins/<name>/`.
4. Removes the entry from `plugins.lock.json`.

`macrift plugin update [<name>]` re-runs the `git pull → validate → confirm`
flow with a diff of changes since the installed version, and re-runs
`on_install` if the manifest's `version` changed.
...
4. **Trusted list** *(future)*: a curated set of plugins maintained by the
