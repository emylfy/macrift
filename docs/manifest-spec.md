...
The manifest is a friendly _surface_ that desugars into change-units. The journal
...
| `value`  | yes      | typed value. Bools normalize to `true`/`false` (the engine maps `defaults read`'s 1/0 in `audit_default`).                                      |
| `sudo`   | optional | `true` forces sudo. Even without it, `_defaults_cmd` auto-retries with sudo on a permission failure, so this is a hint, not a hard requirement. |
...
subdicts via PlistBuddy (`_finder_sort_write`).
...
| `value` | yes      | bool. **Engine mapping:** `value=true → %00` (sound on), `value=false → %01` (muted). Friendly surface key is `boot.startup_sound` (decided — see Decisions made). |
...
| Field   | Required | Notes                                                    |
| :------ | :------- | :------------------------------------------------------- |
| `value` | yes      | bool. `true` → `nohidden` (visible), `false` → `hidden`. |
...
File copy with backup (`copy_config` → `backup_file`).
...
```json
{
	...
	"plist": [{ "domain": "com.googlecode.iterm2", "file": "iterm2.plist" }],
...
- **dotfile backups** — `backup_file` uses `cp -n`, keeping the _first_ `.bak`
...
The `id` field stays empty by design (deferred — YAGNI): journal, manifest, and
drift all key on `domain`/`key`/`dest`, and nothing consumes a slug yet. `label`
is the human reference until a concrete need appears (e.g. a key renamed across
macOS releases).
