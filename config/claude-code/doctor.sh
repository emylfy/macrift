...
ok()   { printf '%b[OK]%b   %s\n'   "$G" "$N" "$*"; ok_count=$((ok_count + 1)); }
miss() { printf '%b[MISS]%b %s\n'   "$Y" "$N" "$*"; miss_count=$((miss_count + 1)); }
...
if [[ ! -f "$claude_md" ]]; then
    ...
    while IFS= read -r line; do
        ...
        total=$((total + 1))
        [[ -f "$target" ]] || { fail "@$rule_name.md → $target not found"; broken=$((broken + 1)); }
    done < "$claude_md"
...
# Statusline (settings.json invokes via `bash $path`, +x is cosmetic)
section "Statusline"
sl_path="$CLAUDE_DIR/statusline.sh"
if [[ -f "$sl_path" ]]; then
    ok "$sl_path"
else
    miss "$sl_path not installed"
fi
