...
- When asking the user to run a command they need to execute themselves (e.g., Claude lacks permission), do all three:
  ...
  3. Tell them to run it by typing their run-alias — `r` by default (the alias macrift adds for `bash /tmp/cmd.sh`; some rename it via `CC_RUN_ALIAS`) — or `bash /tmp/cmd.sh` directly if no alias is set.
