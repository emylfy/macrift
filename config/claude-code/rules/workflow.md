...
- When asking the user to run a command they need to execute themselves (e.g., Claude lacks permission), do all three:
  ...
  3. Tell them to run it by typing `r` in their terminal (their pre-configured alias for `bash /tmp/cmd.sh`), or run `bash /tmp/cmd.sh` manually if the alias isn't installed.
