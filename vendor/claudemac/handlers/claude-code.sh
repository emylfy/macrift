...
CC_CONFIG="${CC_CONFIG:-$MACRIFT_DIR/config/claude-code}"  # plugin menu.sh sets this pre-source
...
claude_code_menu() {
  ...
  while true; do
    ...
    case "$choice" in
      1) _cc_custom_menu ;;
      2) _cc_telegram_menu ;;   # telegram.sh is already sourced by the plugin's menu.sh
      3)
        _cc_reset
        wait_enter
        ;;
...
_cc_settings_apply() {
  ...
  if [[ ! -f "$source" ]]; then
    log_err "No user settings found in $CC_CONFIG/settings/"
    return
  fi
...
_cc_install_component() {
  ...
  if [[ ! -d "$source_dir" ]]; then
    log_err "No $label found in $CC_CONFIG/$dir_name/"
    wait_enter
    return
  fi
...
_cc_install_env() {
  ...
  if [[ ! -f "$source" ]]; then
    log_err "No env.sh found in $CC_CONFIG/"
    return
  fi
...
_cc_reset() {
  ...
  # constants + cleanup fn live in handlers/telegram.sh, already sourced by
  # the plugin's menu.sh, so no source is needed here.
